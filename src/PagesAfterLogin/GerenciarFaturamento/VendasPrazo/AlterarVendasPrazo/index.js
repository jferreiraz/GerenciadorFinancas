...
const AlterarVendasPrazo = (props) => {
  ...
  const pegarDadosID = async (id) => {
    const dbRef = firebase.firestore().collection("usuarios").doc(firebase.auth().currentUser.uid).collection('vendas prazo').doc(id);
  ...
  const deletarDados = async () => {
    setCarregar(true)
    const dbRef = firebase.firestore().collection("usuarios").doc(firebase.auth().currentUser.uid).collection('vendas prazo')
      //.collection("vendas prazo")
...
export default AlterarVendasPrazo;